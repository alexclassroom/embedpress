...
?>
    ...
    private static function generate_legacy_embed_code($attributes, $legacy_config)
    {
        ...
        if (isset($attributes['viewerStyle']) && $attributes['viewerStyle'] === 'flip-book') {
            ...
            $embed_code = '<iframe title="' . esc_attr($iframe_title) . '" class="embedpress-embed-document-pdf ' . esc_attr($id) . '" style="' . esc_attr($legacy_config['dimension']) . '; max-width:100%; display: inline-block" src="' . esc_url(EMBEDPRESS_URL_ASSETS . 'pdf-flip-book/viewer.html?file=' . $src) . '" frameborder="0" oncontextmenu="return false;"></iframe> ';
        }
