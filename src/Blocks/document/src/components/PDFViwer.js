...
const PDFViewer = ({ href, id, width, height, setFetching, unitoption, title }) => {
	return (
		<div className={`embedpress-embed-document-pdf ${id}`} style={{ height: height + 'px', width: width + unitoption }} data-emid={id}>
			<embed title={title} src={sanitizeUrl(href)} style={{ height: height + 'px', width: width + unitoption, maxWidth: '100%' }} onLoad={() => setFetching(false)} />
		</div>
	)
};
